...
function dependecies(){
  tput civis
  clear; dependecies=(aircrack-ng macchanger)

  echo -e "${yellowColour}[*]${endColour}${grayColour} Comprobando programas necesarios...${endColour}"
  sleep 2

  for program in "${dependecies[@]}"; do
      echo -ne "\n${yellowColour}[*]${endColour}${blueColour} Herramientas${endColour}${purpleColour} $program${endColour}${blueColour}... ${endColour}"
...
function startAttack(){
  ...
  if [ "$(echo $attack_mode)" == "Handshake" ]; then
    ...
    xterm -hold -e "airodump-ng ${networkCard}mon" & # coloque 'bspc rule -a XTerm: state=floating' al final de su bspwmrc (si es el caso)
    ...
    clear; echo -e "\n${yellowColour}[*]${endColour}${grayColour} Iniciando ClientLess PKMID Attack...${endColour}\n"
    sleep 2
    ...
    if [ "$(echo $?)" == "0" ]; then
        ...
        echo -e "\n${redColour}[!]${endColour} No se ha podido capturar el paquete necesario...${endColour}\n"
        sleep 2
      fi
  else
